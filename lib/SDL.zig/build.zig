...
pub fn link(sdk: *Sdk, exe: *LibExeObjStep, linkage: LibExeObjStep.Linkage) void {
    ...
    if (target.os.tag == .linux and !is_native) {
        ...
        if (target.abi == .msvc) {
            ...
            const sdl_file_name = switch (linkage) {
                .static => "libSDL2.a",
                .dynamic => "libSDL2.dll.a",
            };

            const sdl_lib_path = std.fs.path.join(b.allocator, &[_][]const u8{
                sdk_paths.libs,
                sdl_file_name,
            }) catch @panic("out of memory");

            exe.addObjectFile(.{ .path = sdl_lib_path });

            const sdl_image_file_name = switch (linkage) {
                .static => "libSDL2_image.a",
                .dynamic => "libSDL2_image.dll.a",
            };

            const sdl_image_lib_path = std.fs.path.join(b.allocator, &[_][]const u8{
                sdk_paths.libs,
                sdl_image_file_name,
            }) catch @panic("out of memory");

            exe.addObjectFile(.{ .path = sdl_image_lib_path });
